...
def setup(bot):
    if not bot.memory.contains('country_zz_zone'):
        bot.memory['country_zz_zone'] = willie.tools.WillieMemory()
    ...
    if not bot.memory.contains('country_iso_3166'):
        bot.memory['country_iso_3166'] = willie.tools.WillieMemory()
    ...
    if not bot.memory.contains('country_ip_cache'):
        bot.memory['country_ip_cache'] = willie.tools.WillieMemory()
    if not bot.memory.contains('country_host_timestamp'):
        bot.memory['country_host_timestamp'] = willie.tools.WillieMemory()
...
def country(bot, trigger):
    ...
    bot.say('A wild ' + trigger.nick + ' arrives from ' + country + '!')
